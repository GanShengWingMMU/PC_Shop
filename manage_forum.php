...
if (isset($_GET['pin_post'])) {
    ...
    $val = intval($_GET['val']); // 1 for Pin, 0 for Unpin
...
if (isset($_GET['scan_post'])) {
    ...
    if ($row = $res->fetch_assoc()) {
        $content = $row['content'];
        $clean_content = str_ireplace($bad_words, '***', $content); // 替换为 ***
...
if (isset($_GET['ban_user'])) {
    ...
    $status = $_GET['status']; // 'Banned' 或 'Active'
...
<head>
    ...
    <style>
        ...
        .post-content { max-width: 300px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; color: #888; font-size: 12px; margin-top: 5px; }
        ...
        /* 🌟 重构的 Action Button 样式，使其看起来更整齐统一 */
        ...
        .action-btn { 
            ...
            font-size:11px; font-weight:bold; transition:0.3s; cursor: pointer; 
                ...
                <tbody>
                    ...
                    if ($res && $res->num_rows > 0) {
                        while ($row = $res->fetch_assoc()) {
                            ...
                            // 🌟 栏位 4: 审核命令 (采用整齐的 Flex 纵向排列，宽度统一)
